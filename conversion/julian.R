...
julian_from_fixed <- function(date) {
  ...
  correction <- ifelse(date < fixed_from_julian(julian_date(year, 3, 1)),
                      0,
                      ifelse(julian_leap_year(year), 1, 2))
...
fixed_from_roman <- function(r_date) {
  ...
  base_date <- switch(as.character(event),
                     ...
                     "3" = fixed_from_julian(julian_date(year, month, ides_of_month(month))))  # IDES
  
  base_date - count + 
    ifelse(julian_leap_year(year) && month == MARCH && event == KALENDS && count >= 6 && count <= 16,
          0, 1) + 
    ifelse(leap, 1, 0)
}
