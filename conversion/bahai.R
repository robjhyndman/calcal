...
bahai_from_fixed <- function(date) {
  ...
  years <- g_year - start - ifelse(date <= fixed_from_gregorian(gregorian_date(g_year, MARCH, 20)), 1, 0)
  ...
  month <- if (date >= fixed_from_bahai(bahai_date(major, cycle, year, 19, 1))) {
    19  # Last month of year
  } else if (date >= fixed_from_bahai(bahai_date(major, cycle, year, AYYAM_I_HA, 1))) {
...
BAHAI_LOCATION <- location(angle(35.696111, 0, 0), angle(51.423056, 0, 0), mt(0), hr(3.5))
...
fixed_from_astro_bahai <- function(b_date) {
  ...
  if (month == 19) {  # Last month of year
    astro_bahai_new_year_on_or_before(BAHAI_EPOCH + floor(MEAN_TROPICAL_YEAR * (years + 0.5))) - 20 + day
  } else if (month == AYYAM_I_HA) {  # Intercalary month, between 18th & 19th
    astro_bahai_new_year_on_or_before(BAHAI_EPOCH + floor(MEAN_TROPICAL_YEAR * (years - 0.5))) + 341 + day
  } else {  # Regular month
    astro_bahai_new_year_on_or_before(BAHAI_EPOCH + floor(MEAN_TROPICAL_YEAR * (years - 0.5))) + 
    (month - 1) * 19 + day - 1
  }
}
