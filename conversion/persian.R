...
fixed_from_persian <- function(p_date) {
  ...
  new_year <- persian_new_year_on_or_before(
    PERSIAN_EPOCH + 180 + floor(MEAN_TROPICAL_YEAR * ifelse(year > 0, year - 1, year))
  )
  
  new_year - 1 +                       # Days in prior years
  ifelse(month <= 7,                   # Days in prior months this year
         (month - 1) * 31,              # First 7 months have 31 days
         (month - 1) * 30 + 6) +        # Remaining months have 30 days
...
fixed_from_arithmetic_persian <- function(p_date) {
  ...
  PERSIAN_EPOCH - 1 +                                   # Days before epoch
