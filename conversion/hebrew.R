...
last_day_of_hebrew_month <- function(h_year, h_month) {
  if (h_month %in% c(IYYAR, TAMMUZ, ELUL, TEVET, ADARII) ||
...
hebrew_new_year <- function(h_year) {
  HEBREW_EPOCH + hebrew_calendar_elapsed_days(h_year) + 
...
hebrew_from_fixed <- function(date) {
  ...
  month <- next_value(start, function(m) {
    date <= fixed_from_hebrew(hebrew_date(
      year, m, last_day_of_hebrew_month(year, m)
    ))
  })
...
yahrzeit <- function(death_date, h_year) {
  ...
  if (death_month == MARHESHVAN && death_day == 30 && 
  ...
  } else if (death_month == KISLEV && death_day == 30 && 
  ...
  } else if (death_day == 30 && death_month == ADAR && 
