test_that("hindu_solar", {
  ...
  expect_equal(
    hindu_solar_date(5126, 3, 1:10) |>
      cal_hindu_solar$from_rd() |>
      cal_hindu_solar$to_rd(),
    hindu_solar_date(5126, 3, 1:10) |> vctrs::vec_data()
  )
  expect_no_error(
...
test_that("hindu_lunar", {
  ...
  expect_equal(
    hindu_lunar_date(5126, 3, FALSE, 1:10) |>
      cal_hindu_lunar$from_rd() |>
      cal_hindu_lunar$to_rd(),
    hindu_lunar_date(5126, 3, FALSE, 1:10) |> vctrs::vec_data()
  )
  expect_no_error(
    as_hindu_lunar(1:1e5) |> as.list() |> validate_hindu_lunar()
  )
})
